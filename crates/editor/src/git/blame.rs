...
struct GitBlameBuffer {
    ...
    commit_details: FxHashMap<Oid, ParsedCommitMessage>,
}

pub struct GitBlame {
    project: Entity<Project>,
    buffer: Entity<MultiBuffer>,
    buffers: FxHashMap<BufferId, GitBlameBuffer>,
    task: Task<Result<()>>,
    focused: bool,
    generated: bool,
...
impl GitBlame {
    ...
    fn sync(&mut self, cx: &mut App, buffer_id: BufferId) {
        ...
        let blame_buffer = match self.buffers.entry(buffer_id) {
            Entry::Occupied(occupied_entry) => occupied_entry.into_mut(),
            Entry::Vacant(vacant_entry) => vacant_entry.insert(GitBlameBuffer {
                entries: SumTree::from_item(
                    GitBlameEntry {
                ...
                buffer_snapshot: buffer.read(cx).snapshot(),
                buffer_edits: buffer.update(cx, |buffer, _| buffer.subscribe()),
                commit_details: Default::default(),
            }),
        };
    ...
    fn generate(&mut self, cx: &mut Context<Self>) {
        ...
        let blame = self.project.update(cx, |project, cx| {
            self.buffer
                ...
                .into_iter()
                .map(|id| {
                    let mbuffer = self.buffer.read(cx);
                    let buffer = mbuffer.buffer(id).unwrap();
                    let snapshot = buffer.read(cx).snapshot();
                    let ebuffer = mbuffer.buffer(id);
                    let buffer_edits = buffer.update(cx, |buffer, _| buffer.subscribe());

                    let buffer = match ebuffer {
                        Some(b) => b,
                        None => return (id, snapshot, buffer_edits, Task::ready(Ok(None))),
                    };
                    let blame_buffer = project.blame_buffer(&buffer, None, cx);
                    (id, snapshot, buffer_edits, blame_buffer)
                })
                .collect::<Vec<_>>()
        });
        ...
        self.task = cx.spawn(async move |this, cx| {
            ...
            this.update(cx, |this, cx| {
                for (id, snapshot, buffer_edits, entries, commit_details) in result {
                    ...
                    match this.buffers.entry(id) {
                        Entry::Occupied(this) => {
                            let this = this.into_mut();
...
) -> FxHashMap<Oid, ParsedCommitMessage> {
    let mut commit_details = FxHashMap::default();
